...
* [ ] Runner
** [ ] If workflow finished (no more steps possible), run would start execution on same workflow from the beginning. This might be confusing: figure out if we should just show that workflow finished and handle restarts in the UI?
*** [ ] If Assignment (rename!) is finished, we need to reject run()?
*** [ ] UIState should check if runner is finished and create a new runner
*** [ ] What do we do with the previous Assignment history?
